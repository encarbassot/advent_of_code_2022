...
function coa(inp){
    ...
    for(let i=0;i<grid.length;i++){//up to down
        //From the left
        let line = grid[i]


        for(let j=0;j<line.length;j++){//left to right
            let tree = line[j]//grid[i][j]

            scenic[i][j]=checkTree(grid,j,i)
    ...
    console.table(scenic)

    total =Math.max(...scenic.map(x=>Math.max(...x)))
